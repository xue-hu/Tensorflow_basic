...
print(x.eval()," ",y.eval(),": ",sess.run(out))
...
x = tf.constant([[0, -2, -1], [0, 1, 2]],tf.int32)
y = tf.zeros_like(x)
out = tf.equal(x,y)
print(sess.run(out))
...
z = tf.reduce_mean( diff )
def mse(): return tf.losses.mean_squared_error(x,y)
def abs(): return tf.losses.absolute_difference(x,y)
out = tf.cond(tf.less(z,0),mse,abs)
print(sess.run(out))
