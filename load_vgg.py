...
class VGG(object):
    ...
    def conv2d_relu(self, prev_layer, layer_idx, layer_name):
        ...
        ###############################
        ## TO DO
        w_init, b_init = self._weights(layer_idx, layer_name)
        ...
        with tf.variable_scope(layer_name, reuse=tf.AUTO_REUSE) as scope :
            w = tf.get_variable('weight', dtype=tf.float32, trainable=False, initializer=w_init)
            b = tf.get_variable('bias', dtype=tf.float32, trainable=False, initializer=b_init)
    ...
    def avgpool(self, prev_layer, layer_name):
        ...
        ## TO DO
        with tf.variable_scope( layer_name, reuse=tf.AUTO_REUSE) as scope :
            out = tf.nn.avg_pool(prev_layer, [1,2,2,1] , [1, 2, 2, 1], padding='VALID', name='pol')
        ###############################
        setattr(self, layer_name, out)
