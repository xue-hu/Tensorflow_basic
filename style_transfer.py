...
class StyleTransfer(object):
    def __init__(self, content_img, style_img, img_width, img_height):
        ...
        self.content_w = tf.constant( 0.001, dtype=tf.float32, name='content_w')
        self.style_w = 1 - self.content_w
    ...
    def _content_loss(self, P, F):
        ...
        ###############################
        ## TO DO
        sum = tf.reduce_sum( tf.square(F-P) )
        self.content_loss = sum / (4.0 *self.img_width *self.img_height)
        ###############################
    ...
    def _single_style_loss(self, a, g):
        ...
        single_style_loss = tf.reduce_sum( tf.square(G - A)) / (4. * pow(N,2) * pow(M,2) )
    ...
    def _style_loss(self, A):
        ...
        for g, styLr, w in zip(G, A, self.style_layer_w):
            self.style_loss += w * self._single_style_loss(styLr, g)
        ###############################
    ...
    def optimize(self):
        ...
        self.opt = tf.train.AdamOptimizer(self.lr).minimize(self.total_loss, var_list= self.input_img, global_step= self.gstep)
        ###############################
    ...
    def train(self, n_iters):
        ...
        with tf.Session() as sess:
            ...
            ###############################
            sess.run(tf.global_variables_initializer())
            writer = tf.summary.FileWriter('./graphs/style_tansfer/', sess.graph)
            ...
            # if tf.train.checkpoint_exists('checkpoint_dict/StyleTransfer') :
            #     saver.restore(sess, 'checkpoint_dict/StyleTransfer')
            ...
            for index in range(initial_step, n_iters):
                ...
                if (index + 1) % skip_step == 0:
                    ...
                    if (index + 1) % 20 == 0:
                        ...
                        ###############################
                        saver.save(sess,'checkpoint_dict/StyleTransfer', global_step= self.gstep)

if __name__ == '__main__':
    setup()
    machine = StyleTransfer('content/deadpool.jpg', 'styles/pattern.jpg', 256, 256)
    machine.build()
    machine.train(30)